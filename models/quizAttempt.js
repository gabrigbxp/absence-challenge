...
QuizAttemptSchema.pre("validate", function (next) {
  if (!this.questions.length) return next(new BusinessError(400, "QuizAttempt need at least 1 question"))

  for (let question of this.questions) {
    if (!question.answers.length) return next(new BusinessError(400, "QuizAttempt need at least 1 answer per question"))
  }
